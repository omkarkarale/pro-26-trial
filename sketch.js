var b_grp, t_grp, f_grp, mov_block;
...
function setup() {
  ...
  player = createSprite(1750,500,10,10);
...
function draw() {
  ...
  // if(player.isTouching(mov_block)){  
    if(player.collide(mov_block) && mov_block.velocityX === -5){
      player.velocityX = -5;
      console.log("left")
    }
    else if(player.collide(mov_block) && mov_block.velocityX === 5){
      player.velocityX = 5;
      console.log("right")
    }
  // }

  else if(player.isTouching(b_grp)){
    player.velocityX = 0;
    console.log("n_mov");
  }
  player.collide(b_grp);
  player.collide(f_grp);
  f_grp.bounceOff(b_grp);
  drawSprites();    
}
...
function Trampoline(x,y){
 ...
 function Mov_block(x,y){
  ...
  mov_block.debug = true;
  mov_block.velocityX = -5;
  f_grp.add(mov_block);
 }
